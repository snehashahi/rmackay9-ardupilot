...
void Mode::calc_steering_to_waypoint(const struct Location &origin, const struct Location &destination, bool reversed)
{
    ...
    if (rover.use_pivot_steering(_yaw_error_cd)) {
        // use angle error controller
        const float yaw_error_rad = wrap_PI(radians(_yaw_error_cd * 0.01f));
        const float steering_out = attitude_control.get_steering_out_angle_error(yaw_error_rad, g2.motors.have_skid_steering(), g2.motors.limit.steer_left, g2.motors.limit.steer_right);
