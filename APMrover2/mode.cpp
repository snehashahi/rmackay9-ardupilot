...
float Mode::calc_reduced_speed_for_turn_or_distance(float desired_speed)
{
    ...
    float line_yaw_diff = wrap_180_cd(get_bearing_cd(_origin, _destination) - ahrs.yaw_sensor);
    float crosstrack_error = rover.nav_controller->crosstrack_error();
    float dist_from_line = fabsf(crosstrack_error);
    bool heading_away = is_positive(line_yaw_diff) == is_positive(crosstrack_error);
    ...
    if (counter > 10) {
        counter = 0;
        ::printf("aw:%d di:%4.2f lat:%4.2f wpo:%4.2f tur:%4.2f radm:%4.2f spd:%4.2f pspd:%4.2f\n",
                (int)heading_away,
                (double)dist_from_line,
                (double)lata,
                (double)(rover.g.waypoint_overshoot + wp_overshoot_adj),
                (double)degrees(turn_angle_rad),
                (double)radius_m,
                (double)des_speed,
                (double)speed_scaled
                );
