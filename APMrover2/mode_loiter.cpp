...
{
    // get distance (in meters) destination
    ...
    if (_distance_to_destination <= g.waypoint_radius) {
        ...
        _yaw_error_cd = _desired_yaw_cd - ahrs.yaw_sensor;
        ...
        float yaw_error_ratio = constrain_float(fabsf(_yaw_error_cd / 9000.0f), 0.0f, 1.0f) * ((100.0f - g.speed_turn_gain) * 0.01f);
