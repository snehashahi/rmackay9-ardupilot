...
float Sailboat::get_turn_circle(float steering_deg) const
{
    if (is_zero(steering_deg)) {
        return 0;
    }
    return turning_circle * sinf(radians(max_wheel_turn)) / sinf(radians(steering_deg * max_wheel_turn));
}

// return yaw rate in deg/sec given a steering angle in degrees and speed in m/s
float Sailboat::get_yaw_rate(float steering_deg, float speed) const
{
    if (is_zero(steering_deg) || is_zero(speed)) {
        return 0;
    }
    float d = get_turn_circle(steering_deg);
    float c = M_PI * d;
    float t = c / speed;
    float rate = 360.0f / t;
    return rate;
...
float Sailboat::get_lat_accel(float steering_deg, float speed) const
{
    float yaw_rate = get_yaw_rate(steering_deg, speed);
