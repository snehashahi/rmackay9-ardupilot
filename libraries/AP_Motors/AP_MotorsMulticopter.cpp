...
void AP_MotorsMulticopter::update_battery_resistance()
{
    ...
    if (!_flags.armed) {
        ...
        if ((_batt_timer < 400) && ((_batt_current_resting*2.0f) < _batt_current)) {
            if (get_throttle() >= get_throttle_hover()*AP_MOTORS_BATT_THST_THRESH) {
        ...
        if(batt_resistance < _batt_resistance && get_throttle() >= get_throttle_hover()*AP_MOTORS_BATT_THST_THRESH){
            _batt_resistance = batt_resistance;
        }
    }
}
