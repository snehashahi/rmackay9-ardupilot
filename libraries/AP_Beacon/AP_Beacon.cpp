...
void AP_Beacon::update_boundary_points()
{
    ...
    while (!boundary_success && !boundary_failure) {
        ...
        uint8_t next_idx;
        if (get_next_boundary_point(beacon_points, num_beacons, curr_beacon_idx, next_idx)) {
...
bool AP_Beacon::get_next_boundary_point(const Vector2f* boundary_pts, uint8_t num_points, uint8_t current_index, uint8_t& next_index)
{
    // sanity check
