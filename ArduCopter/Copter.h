...
# include USERHOOK_VARIABLES
    ...
    typedef union {
        struct {
            ...
            bool pre_arm_rc_check        : 1; // true if rc input pre-arm checks have been completed successfully
            bool pre_arm_check           : 1; // true if all pre-arm checks (rc, accel calibration, gps lock) have been performed
            bool auto_armed              : 1; // stops auto missions from beginning until throttle is raised
            bool logging_started         : 1; // true if dataflash logging has started
            bool land_complete           : 1; // true if we have detected a landing
            bool new_radio_frame         : 1; // Set true if we have new PWM data to act on from the Radio
            bool usb_connected           : 1; // true if APM is powered from USB connection
            bool rc_receiver_present     : 1; // true if we have an rc receiver present (i.e. if we've ever received an update
            bool compass_mot             : 1; // true if we are currently performing compassmot calibration
            bool motor_test              : 1; // true if we are currently performing the motors test
            bool initialised             : 1; // true once the init_ardupilot function has completed.  Extended status to GCS is not sent until this completes
            bool land_complete_maybe     : 1; // true if we may have landed (less strict version of land_complete)
            bool throttle_zero           : 1; // true if the throttle stick is at zero, debounced, determines if pilot intends shut-down when not using motor interlock
            bool gps_glitching           : 1; // true if GPS glitching is affecting navigation accuracy
            bool using_interlock         : 1; // aux switch motor interlock function is in use
            bool motor_emergency_stop    : 1; // motor estop switch, shuts off motors when enabled
            bool land_repo_active        : 1; // true if the pilot is overriding the landing position
            bool motor_interlock_switch  : 1; // true if pilot is requesting motor interlock enable
            bool in_arming_delay         : 1; // true while we are armed but waiting to spin motors
            bool initialised_params      : 1; // true when the all parameters have been initialised. we cannot send parameters to the GCS until this is done
            bool compass_init_location   : 1; // true when the compass's initial location has been set
            bool rc_override_enable      : 1; // aux switch rc_override is allowed
        };
        uint32_t value;
    } ap_t;
